...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.pages.addWidget(self.page_about)     # 3
    ...
    def _set_nav_checked(self, which: str):
        for b in [self.btn_home, self.btn_live, self.btn_settings, self.btn_about]:
            b.setChecked(False)
    ...
    def _build_settings_page(self) -> QWidget:
        ...
        og.addWidget(QLabel("Hold for 'CIBLE'"), 0, 0); og.addWidget(self.s_hold, 0, 1)
        og.addWidget(QLabel("Detect every N frames"), 1, 0); og.addWidget(self.s_every, 1, 1)
        og.addWidget(QLabel("Camera index"), 2, 0); og.addWidget(self.s_cam_index, 2, 1)
    ...
    def camera_tick(self):
        ...
        if self.cfg.opencv_detect and self.chk_detect.isChecked() and (not self.face_cascade.empty()):
        ...
        if detected_obj and elapsed >= self.cfg.hold_seconds:
            cv2.putText(frame, "CIBLE DETECTEE", (16, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (239, 68, 68), 3)
        else:
            cv2.putText(frame, f"Hold: {elapsed:.1f}s/{self.cfg.hold_seconds:.1f}s", (16, 70),
